...
(function () {
  ...
  if (Config.$crossFrame) {
    try {
      while (
        dptop.parent != dptop &&
        dptop.parent[d][tag]('frameset').length == 0
      ) {
        dptop = dptop.parent;
      }
    } catch (e) {}
  ...
  function getWH(w) {
    ...
    var doc = w[d],
      width = w.innerWidth
        ? w.innerWidth
        : doc[de] && doc[de].clientWidth
        ? doc[de].clientWidth
        : doc.body.offsetWidth,
      height = w.innerHeight
        ? w.innerHeight
        : doc[de] && doc[de].clientHeight
        ? doc[de].clientHeight
  ...
  function getScroll(w) {
    ...
    doc =
      doce &&
      doce.scrollTop != null &&
      (doce.scrollTop > db.scrollTop || doce.scrollLeft > db.scrollLeft)
        ? doce
        : db;
  ...
  function main(initcfg, preLoad) {
    ...
    if (preLoad) {
      ...
      try {
        if (
          !cfg.el ||
          cfg.el['My97Mark'] === true ||
          cfg.el.disabled ||
          ($dp.dd && display($dp.dd) != 'none' && $dp.dd.style.left != '-970px')
        ) {
      ...
      if (evt && cfg.el.nodeType == 1 && !compareCfg(cfg.el.initcfg, initcfg)) {
        ...
        dpAttachEvent(
          cfg.el,
          evt.type == 'focus' ? 'onclick' : 'onfocus',
          function () {
            main(initcfg);
          }
        );
  ...
  function showPicker(cfg, preLoad) {
    ...
    if (
      preLoad ||
      cfg.eCont ||
      new RegExp(/input|textarea|div|span|p|a/gi).test(nodeName)
    ) {
    ...
    if (
      ...
      cfg.eCont ||
      ($dp.dd &&
        (cfg.getRealLang().name != $dp.dd.lang || cfg.skin != $dp.dd.skin))
    ) {
      if (cfg.eCont) {
    ...
    function ddInit(cont, cfg) {
      ...
      function ddWrite() {
        ...
        var h = [
          ...
          'if(!$dp.isTouch)doc.oncontextmenu=function(){try{$c._fillQS(!$dp.has.d,1);showB($d.qsDivSel);}catch(e){};return false;};',
          '</script><script src=',
          jsPath,
          'lang/',
          realLang.name,
    ...
    function setPos(dp) {
      ...
      if (isNaN(t)) t = 0;
      if (
        mm.topM + objxy.bottom + ddHeight > currWH.height &&
        mm.topM + objxy.top - ddHeight > 0
      ) {
      ...
      l +=
        scr.left +
        Math.min(mm.leftM + objxy.left, currWH.width - ddWidth - 5) -
        ($IE ? 2 : 0);
